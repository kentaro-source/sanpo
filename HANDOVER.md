...
最終更新: 2026-08-01（第14セッション継続。記載の数値・挙動はすべてコード読解または実機/devでの実測で確認済み。推測は「疑い」と明記）

## 0. 2026-07-30 に起きたこと（最初に読む）

**Google Cloud の無料トライアル（$300/90日）が終了して請求が無効になり、地図が全く出なくなった。** ユーザーが有料アカウントを有効化して復旧。その後、課金対策と実機検証を一気に行った。

- **料金体系（公式ページで確認）**: Maps Platform は請求先が有効な限り**期限なしで毎月**リセットされる無料枠がある。Dynamic Maps 10,000回/月・Directions 10,000回/月・Geocoding 10,000回/月。超過分は $7 / $5 per 1,000
- **課金の実害の正体**: このアプリは**1回の起動で経路検索を約42回**呼んでいた（実測: Directions 541回 ÷ Maps JS 13回）。原因は下記「Directions キャッシュ全消しバグ」。1日15回起動で月18,000回=無料枠超過（約$45/月）だった
- **上限設定（ユーザーが実施済み）**: Cloud Console → Google Maps Platform → 割り当て → 対象APIを選択 → 「Requests per day」の⋮ →「割り当てを編集」。**Directions を 300回/日に設定済み**（300×31=9,300 < 10,000 なので構造的に課金不能）。Geocoding は v3/v4 に分かれており未設定の可能性あり（現在61回なので実害なし）。Maps JavaScript も未設定（13回）
- **重要**: 予算アラートは通知のみで支払いを止めない（公式明記）。止まるのは割り当て上限のみ
- **上限が実際に機能することを実証済み（2026-07-30〜31）**: 事前生成の大量呼び出し中に生成器が `quotaHit: true` で安全停止 → 以後は継続的に拒否 → 日付が変わって `OK` に回復。数秒で戻る短期レート制限とは異なる挙動。**注意**: 短期レート制限も同じ `OVER_QUERY_LIMIT` を返すため、1回の観測で「日次上限に達した」と断定してはいけない（一度誤診した）。判別は「時間を空けても拒否が持続するか」
- **事前生成の完了により、上限に達してもアプリの地図は正常に描ける**（実機で確認）
- **APIキーは1本を PWA と APK で共用**しており、referrer に localhost を許可しているため実質的に誰でも使える状態（§5 レビュー指摘1）

**実機（OnePlus CPH2719）は 2026-06-30 版のまま1ヶ月放置されていた**ため、今日までの修正が全て未反映だった。本日 APK を再ビルドして導入し、保留3件を実機で確認済み（下記）。
...
| 都市（cities.ts） | **1,023**（うち 958 が route 上に km 解決。200km 圏外の 65 は立寄ボーナス対象外） |
| セグメント | 193（segmentMeta.ts で分類済み **192** + 最終首都→東京の wrap） |
| 総距離 | **362,491 km**（2026-08-01 の重複区間解消で 369,554→362,491 に減少。great-circle×1.4 の raw スケール。ルート変更のたび変動） |
| squares | 18,131（~20km 間隔。`positionAtKm` の補間精度を決める） |
...
### ✅ 2026-07-30 に実機で解決を確認した項目（旧「未解決」から移動）

すべて OnePlus CPH2719 に本日の APK を導入して CDP で実測。

1. **X投稿の位置化け — 解決**。share 文面が「🇮🇩 インドネシア Klungkung Regency → Kabupaten Karangasem」（バリ島東部）で、地図マーカー・停車地リスト（次=マタラム）と一致。以前の「マーカーは中国なのに表示はフィリピン」の乖離は再現しない
2. **香港/マカオ/台湾の入国審査 — 発火済み**。`borderRollsWon` に HK・MO・TW が入り **11/196**（首都8＋独立3）
3. **島伝いルート — 稼働中**。停車地に マタラム→スンバワブサール→ビマ→ラブアンバジョ が出る
4. **Directions キャッシュ全消しバグ — 解決**。旧ビルドの残骸（負キャッシュ103件＋未簡略化2.25MBの巨大エントリ）を一度だけ掃除する処理を入れ、実機で **2.25MB→18.6KB / 負103件→0件** を確認
5. **実行時の Directions 呼び出し — ゼロ化を確認**。キャッシュを空にしてコールドスタートしても `liveCacheEntries: 0` のまま地図が正常描画（事前生成データが効いている）。**42回/起動 → 0回**

### ❌ 未解決（2026-07-30 時点）

1. **コールドスタート直後の自動センタリングが発火しない** — 地図が外洋のまま。📍ボタンを押すと正しくプレイヤー位置（バリ）に移動する。原因未特定。
   - 本日 2つの原因を潰した: (a) 初期センタリング2箇所が `player.distanceKm`（全体km）を窓内弧長 `cumKm`（0起点）と比較する二重スケールだった (b) マーカー・色分け・センタリングの3箇所に複製されていた `projectOntoPath` フォールバックが、部分パスに投影して**別大陸（マニラ）にスナップ**していた → `playerOnBuiltPath()` に集約し許容距離を検証
   - ただし発火しない症状自体は修正前から同じで、まだ残っている。**次にやるなら実機で `google.maps.Map.prototype.panTo` を差し替えて呼び出し座標とスタックを直接観測する**（推測で追うと時間を無駄にする）
2. ~~事前生成のカバー率~~ — **2026-07-31 に完了**。道路チャンクを持つ全151セグメントを生成（2.1MB、`public/route-geometry/`）。機械検証 `npx tsx scripts/_covcheck.mts` で `missing: []` を確認済み。残り38セグメントは全 leg が海路（島嶼間・大洋横断）で Directions 不要。**プレイヤーが世界一周のどこにいても経路検索APIを呼ばない**
   - ⚠️ この全量入りAPKは**まだ実機に未インストール**（ビルド成功後にUSBが切断された）。次回接続時に `npm run cap:sync` → gradle → `adb install -r` → force-stop → 起動、そのあとキャッシュを空にしてコールドスタートし呼び出しゼロを再確認すること
3. （旧項目）**X投稿の位置化け**の記述は上記「解決」に移動
...
### 実機のゲーム状態
- **2026-07-30 実測（CDP）**: `distanceKm` ≈ 29,508（バリ島東部）。`visitedCapitals` に PH/BN/ID 追加、`borderRollsWon` に HK/MO/TW 追加で **11/196**。旧「フィリピンのまま」は解消し、東南アジア〜インドネシアを順調に前進中
- **2026-08-01 ユーザー申告 + データ照合**: 現在地は**km 45,146（ビャイ手前、ミャンマー）** — MM-PYAY の cityDistances と一致。マレー半島（マラッカ/セレンバン/クアラルンプール）を経て、ミャンマー入りしネピドー/ヤンゴン/ヒンターダを通過済み。端末には本日のルート順序修正 APK は**未反映**（§5 task 1）

---

## 5. 残タスクと次に着手すべきこと（優先順）

1. **本日(2026-08-01)の修正入りAPKのインストール（最優先）**: ルート順序修正17区間 + 事前生成ジオメトリ再生成分が未反映。`npm run cap:sync` → gradle → `adb install -r` → `am force-stop` → 起動 → キャッシュ空でコールドスタートし `liveCacheEntries: 0` と マラッカ→セレンバン→クアラルンプールの順序を再確認
2. ~~ShareToX 位置化け修正の実機確認~~ / ~~香港/マカオ/台湾の入国審査確認~~ / ~~島伝いルート~~ — **すべて解決・実機確認済み（§4「✅ 2026-07-30 に実機で解決を確認した項目」参照）**
3. **ミャンマー→バングラデシュ、インド北東部経由への変更は見送り**（2026-08-01検討）: シットウェー→ダッカのベンガル湾越え(直線)は、実測ではネピドー→カレー→インパール→アガルタラ→ダッカの陸路(全区間1.3〜1.85倍、現実的)の方が地理的に自然。ただし**現在地(km45,146、ビャイ手前)が既にこの区間の途中**(ネピドー/ヤンゴン/ヒンターダ通過済み)のため、「通過済みルートに手を出さない」原則により変更を見送った。プレイヤーがこの区間を過ぎて未来の再検討時に持ち出せるよう記録。ヤンゴンは `visitedInRealLife: true`(思い出ボーナス+6の対象)である点も判断材料
4. **重複参照都市による順序逆転の残り1件**: CN-MN(km8,486〜10,457、大同/フフホト/二連浩特/サインシャンドがMN-PHと重複)は**既に通過済みのため意図的に未修正**。他46都市中42都市は2026-08-01に修正済み(`0ac841d`)。プレイヤーが将来ラップして再訪する場合を除き対応不要
...
2. ~~**[設計・高] 進行データのバックアップ手段がない**~~ — **2026-07-31 実装済み**。☰メニューに「💾 バックアップ」を追加（`exportSaveText`/`importSaveText` in `utils/storage.ts`）。ワンタップでクリップボードへコピー、貼り付けで復元。**壊れた入力では既存セーブを一切書き換えない**（dev検証: 破壊→復元で29,508km完全復旧、不正入力3種を拒否しセーブ無傷）。⚠️ 実機での動作確認は未実施
...
5. ~~**[設計・中] waypoint 参照エラーの黙殺**~~ — **一部対応済み（2026-08-01）**。`scripts/_ordercheck.mts` を追加し、(a) 同一都市が複数セグメントで waypoint 参照される（km 後勝ち上書きで順序破壊）(b) seaSegments の添字が範囲外/短距離すぎる、の2種を機械検出できるようになった。ルート編集時は `npx tsx scripts/_ordercheck.mts` を実行すること。ただし typo による waypoint 未解決（cityDistances 丸ごと欠落）自体の検出は `_routestats.mts` の `cityDistancesResolved` を都市総数と突き合わせる形でまだ未実装
...
### 事前生成ジオメトリの運用（2026-07-31 完成）

- **データ**: `public/route-geometry/<FROM>-<TO>.json`（151ファイル/2.1MB）。中身は「Directionsキャッシュキー → encoded polyline」。APK では同梱されるのでオフラインでも効く
- **なぜ壊れないか**: キーに全stop座標が入るため、ルートを変更するとキーが変わって自動的にミス→ライブ取得にフォールバックする。**古いデータで誤った地図を描く事故が構造的に起きない**
- **ルートを変更したら**: そのセグメントのファイルを削除して生成器を再実行する（消さなくても実害はないが、ライブ取得が走る）
- **生成手順**: dev サーバ起動 → ページで
  ```js
  eval(await (await fetch('/__route-geometry-script')).text());
  window.genRouteGeometry({ maxCalls: 200 });
  ```
  既存ファイルはスキップするので**中断・再開可能**。`OVER_QUERY_LIMIT` で安全停止（部分ファイルを書かない）
- **カバー率の確認**: `npx tsx scripts/_covcheck.mts` → `missing: []` なら完全
- **既知の不便**: 生成中に**ソースファイルを編集すると Vite の HMR でページがリロードし生成器が死ぬ**。`server.watch.ignored` に `public/route-geometry` を入れたが Windows では効いていない様子で、書き込み自体でもリロードが起きる（毎回15〜80セグメントで中断）。再実行すれば続きから走るので実害は小さい
